...
; (async function () {
  ...
  let COMPONENTS_BASE_PATH='./vue-hacker-components';
      ...
      loadVMJsFile(fileName, vmName, useSingleFileVM = false, vmFilePath, isPureReact) {
        ...
        return new Promise(async (resolve, reject) => {
          ...
          if (!res) {
            ...
            if(!path.startsWith(COMPONENTS_BASE_PATH)){
              path=COMPONENTS_BASE_PATH+'/'+path
              // path=COMPONENTS_BASE_PATH+'/'+path
            }
          ...
          try {
            ...
            console.error("eval解析失败--:", e, evalStr);
            console.trace();
          }
      ...
      loadVM(vmName, isMixin = false, path = null) {
        ...
        return new Promise(async (resolve, reject) => {
          ...
          if (result) {
            ...
            let s=await window.Veact.service.loadVMJsFile(fileName, vmName, shouldLoadSingleFileVm, path, isPureReact);
  ...
  let veactReadyPromise=null;
  // let hasVeactReady=false;
  Veact.ready=()=>{
    if(!veactReadyPromise){
      return new Promise((resolve)=>{
        veactReadyPromise=resolve
      })
    }else{
...
`;
  ...
  Veact.COMPONENT_BASE_URL =COMPONENTS_BASE_PATH+'/'
